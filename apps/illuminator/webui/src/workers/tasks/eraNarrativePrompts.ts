/**
 * Era Narrative Prompt Construction
 *
 * System and user prompts for the era narrative pipeline steps:
 * threads, generate, and edit. Extracted from eraNarrativeTask.ts
...
// ============================================================================
// Historian Identity Helpers
// ============================================================================

function formatHistorianIdentity(historianConfig: HistorianConfig): string {
  const privateFacts =
  ...
  return `**Personality:** ${historianConfig.personalityTraits.join(", ")}
...
// ============================================================================
// Thread Synthesis Step Prompts
// ============================================================================

export function buildThreadsSystemPrompt(
  historianConfig: HistorianConfig,
  tone: EraNarrativeTone,
  eraName: string,
  arcDirection?: string
): string {
  const identity = formatHistorianIdentity(historianConfig);
...
// ============================================================================
// Thread User Prompt Helpers
// ============================================================================

function formatWorldContextSections(record: EraNarrativeRecord): string[] {
  const sections: string[] = [];
  const wc = record.worldContext;
  if (!wc) return sections;
...
function formatDynamicsAndIdentities(wc: EraNarrativeRecord["worldContext"]): string[] {
  const sections: string[] = [];
  if (!wc) return sections;
...
interface SourceBriefsResult {
  sections: string[];
  structural: PrepBrief[];
  contextual: PrepBrief[];
  unclassified: PrepBrief[];
}

function formatSourceBriefsResult(prepBriefs: PrepBrief[]): SourceBriefsResult {
  const sections: string[] = [];
  const byYear = (a: PrepBrief, b: PrepBrief) => (a.eraYear || 0) - (b.eraYear || 0);
  ...
  return { sections, structural, contextual, unclassified };
}

export function buildThreadsUserPrompt(record: EraNarrativeRecord): string {
  const sections: string[] = [];
  sections.push(`=== ERA: ${record.eraName} ===`);

  if (record.worldContext) {
    sections.push(...formatWorldContextSections(record));
    sections.push(...formatDynamicsAndIdentities(record.worldContext));
  }

  const briefsResult = formatSourceBriefsResult(record.prepBriefs);
  sections.push(...briefsResult.sections);

  const arcSources =
    briefsResult.structural.length + briefsResult.contextual.length + briefsResult.unclassified.length;
  const hasTiers = briefsResult.structural.length > 0 || briefsResult.contextual.length > 0;
...
// ============================================================================
// Generation Step Prompts
// ============================================================================

export function buildGenerateSystemPrompt(historianConfig: HistorianConfig, eraName: string): string {
  const privateFacts =
    historianConfig.privateFacts.length > 0
      ? `\n**Private knowledge:** ${historianConfig.privateFacts.join("; ")}`
      : "";
...
// ============================================================================
// Generate User Prompt Helpers
// ============================================================================

function formatGenerateWorldContext(record: EraNarrativeRecord): string[] {
  const sections: string[] = [];
  const wc = record.worldContext;
  if (!wc) return sections;
...
function formatGenerateDynamicsAndIdentities(wc: EraNarrativeRecord["worldContext"]): string[] {
  const sections: string[] = [];
  if (!wc) return sections;
...
function formatSynthesisContext(synthesis: NonNullable<EraNarrativeRecord["threadSynthesis"]>): string[] {
  const sections: string[] = [];
...
export function buildGenerateUserPrompt(record: EraNarrativeRecord): string {
...
Year range: ${eraStart}–${eraEnd}`);
  ...
  sections.push(...formatGenerateWorldContext(record));
  sections.push(...formatGenerateDynamicsAndIdentities(record.worldContext));

  if (synthesis) {
    sections.push(...formatSynthesisContext(synthesis));
  }
...
export function buildEditUserPrompt(record: EraNarrativeRecord, contentToEdit: string): string {
  const synthesis = record.threadSynthesis;
  ...
  if (synthesis?.threads?.length) {
    const threadList = synthesis.threads
      .map((t) => `- ${t.name}: register "${t.register}"`)
      .join("\n");
    editSections.push(
...
// ============================================================================
// Cover Image Scene Prompt
// ============================================================================

export function buildCoverImageScenePrompt(
  eraName: string,
  thesis: string,
