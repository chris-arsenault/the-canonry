...
export default function useEntityGuidanceSync({
  externalEntityGuidance, onEntityGuidanceChange,
  externalCultureIdentities, onCultureIdentitiesChange,
}) {
  ...
  useEffect(() => {
    if (externalCultureIdentities !== undefined) {
      pendingCultureIdentitiesRef.current = externalCultureIdentities || createDefaultCultureIdentities();
    }
  }, [externalCultureIdentities]);

  const entityGuidanceSyncTimeoutRef = useRef(null);
  const updateEntityGuidance = useCallback((nextGuidance) => {
  ...
  const updateCultureIdentities = useCallback((nextIdentities) => {
    ...
    if (cultureIdentitiesSyncTimeoutRef.current) clearTimeout(cultureIdentitiesSyncTimeoutRef.current);
  ...
  return {
    entityGuidance: localEntityGuidance, updateEntityGuidance,
    cultureIdentities: localCultureIdentities, updateCultureIdentities,
  };
}
