...
/** Entity context extended with cast role flags */
interface CastContext extends RevisionEntityContext {
  isPrimary?: boolean;
  isLens?: boolean;
  isTertiary?: boolean;
...
/** Assembled context for a chronicle's backport config modal */
export interface BackportConfigContext {
  chronicleId: string;
  chronicleTitle: string;
  entities: CastContext[];
  chronicleText: string;
  perspectiveSynthesisJson: string;
  perEntityStatus: Record<string, string>;
}

type GetEntityContextsForRevision = (entityIds: string[]) => Promise<RevisionEntityContext[]>;
type ReloadEntities = (entityIds: string[]) => Promise<void>;
type SetChronicleRefreshTrigger = Dispatch<SetStateAction<number>>;

export interface UseBackportFlowProps {
  projectId: string | null;
  simulationRunId: string | null;
  getEntityContextsForRevision: GetEntityContextsForRevision;
  reloadEntities: ReloadEntities;
  setChronicleRefreshTrigger: SetChronicleRefreshTrigger;
}

export interface UseBackportFlowReturn {
  backportRun: SummaryRevisionRun | null;
  isBackportActive: boolean;
  backportChronicleId: string | null;
  toggleBackportPatchDecision: (entityId: string, accepted: boolean) => Promise<void>;
  updateBackportAnchorPhrase: (entityId: string, anchorPhrase: string) => Promise<void>;
  cancelBackport: () => void;
  backportConfig: BackportConfigContext | null;
  setBackportConfig: Dispatch<SetStateAction<BackportConfigContext | null>>;
  handleBackportLore: (chronicleId: string) => Promise<void>;
  handleBackportConfigStart: (selectedEntityIds: string[], customInstructions?: string) => void;
  handleAcceptBackport: () => Promise<void>;
  handleMarkEntityNotNeeded: (entityIds: string[]) => Promise<void>;
  bulkBackportProgress: BulkBackportProgress;
  isBulkBackportActive: boolean;
  showBulkBackportModal: boolean;
  handleStartBulkBackport: () => Promise<void>;
  handleConfirmBulkBackport: () => void;
  handleCancelBulkBackport: () => void;
  handleCloseBulkBackport: () => void;
  assembleContextForChronicle: (chronicleId: string) => Promise<BackportConfigContext | null>;
}

// --- Module-level helpers ---

async function buildCastContexts(
  ...
  const castContexts: CastContext[] = baseContexts.map((ctx) => ({
    ...ctx,
...
async function appendTertiaryContexts(
  allContexts: CastContext[],
  chronicle: ChronicleRecord,
  getEntityContextsForRevision: GetEntityContextsForRevision
): Promise<void> {
...
function buildPerspectiveSynthesisJson(chronicle: ChronicleRecord): string {
...
async function resolvePerEntityBackportStatus(
  allContexts: CastContext[],
  chronicleBackportMap: Record<string, EntityBackportEntry>,
  chronicleId: string
): Promise<Record<string, string>> {
  const perEntityStatus: Record<string, string> = {};
...
async function assembleChronicleContext(
  ...
  chronicleId: string,
  getEntityContextsForRevision: GetEntityContextsForRevision
): Promise<BackportConfigContext | null> {
...
async function applyBackportPatches(
...
async function markEntitiesBackported(
  chronicleId: string,
  entityIds: string[],
  setChronicleRefreshTrigger: SetChronicleRefreshTrigger
): Promise<void> {
  const now = Date.now();
  const entries: EntityBackportEntry[] = entityIds.map((id) => ({ entityId: id, status: "backported" as const, updatedAt: now }));
  try {
...
async function getEligibleChronicleIds(simRunId: string): Promise<Array<{ chronicleId: string; chronicleTitle: string; pendingCount: number }>> {
...
async function runMarkEntityNotNeeded(
  ...
  const now = Date.now();
  const entries: EntityBackportEntry[] = entityIds.map((id) => ({ entityId: id, status: "not_needed" as const, updatedAt: now }));
...
interface BulkBackportSetupProps {
  assembleContextForChronicle: (chronicleId: string) => Promise<BackportConfigContext | null>;
  reloadEntities: ReloadEntities;
  setChronicleRefreshTrigger: SetChronicleRefreshTrigger;
  simulationRunId: string | null;
...
interface BulkBackportSetupReturn {
  bulkBackportProgress: BulkBackportProgress;
  isBulkBackportActive: boolean;
  showBulkBackportModal: boolean;
  handleStartBulkBackport: () => Promise<void>;
...
function useBulkBackportSetup({
  ...
  simulationRunId,
  projectId,
}: BulkBackportSetupProps): BulkBackportSetupReturn {
  const applyBulkPatches = useCallback(
    async (patches: SummaryRevisionPatch[], chronicleId: string, sentEntityIds: string[]) => {
      if (patches.length > 0) {
  ...
  const [showBulkBackportModal, setShowBulkBackportModal] = useState<boolean>(false);
...
export function useBackportFlow({
  ...
  reloadEntities,
  setChronicleRefreshTrigger,
}: UseBackportFlowProps): UseBackportFlowReturn {
  const {
    run: backportRun,
  ...
  const [backportConfig, setBackportConfig] = useState<BackportConfigContext | null>(null);
  const backportSentEntityIdsRef = useRef<string[] | null>(null);

  const handleBackportLore = useCallback(
    async (chronicleId: string) => {
  ...
  const handleBackportConfigStart = useCallback(
    (selectedEntityIds: string[], customInstructions?: string) => {
