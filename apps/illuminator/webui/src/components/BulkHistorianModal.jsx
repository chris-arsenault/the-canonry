...
export default function BulkHistorianModal({ progress, onConfirm, onCancel, onClose, onChangeTone, editionMaxTokens }) {
            ...
            <>
              ...
              {isClear ? null : isReview ? (
                ...
                <div style={{
                  ...
                  <span style={{ fontWeight: 600, color: 'var(--text-primary)', marginRight: '6px' }}>Tones cycle:</span>
                  {TONE_CYCLE_ORDER.map((t, i) => {
                    const meta = TONE_META[t];
                    return (
                      <span key={t}>
                        {i > 0 && <span style={{ margin: '0 4px', color: 'var(--text-muted)' }}>&rarr;</span>}
                        <span style={{ color: '#8b7355' }}>{meta?.symbol}</span>
                        {' '}{meta?.label}
                      </span>
                    );
                  })}
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            padding: '4px 10px',
                            borderRadius: '4px',
                            border: isSelected ? '1px solid #8b7355' : '1px solid var(--border-color)',
                            background: isSelected ? 'rgba(139, 115, 85, 0.15)' : 'var(--bg-secondary)',
                  ...
                  {progress.entities.map((entity, i) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: '6px 12px',
                        borderBottom: i < progress.entities.length - 1 ? '1px solid var(--border-color)' : 'none',
                        fontSize: '12px',
                      }}
                    >
                      ...
                      {!isReview && entity.tokenEstimate > 0 && (
                        <span style={{
                          ...
                          color: editionMaxTokens && entity.tokenEstimate > editionMaxTokens
                            ? '#ef4444'
                            : 'var(--text-muted)',
                          flexShrink: 0,
                        }}
              ...
              {!isReview && !isClear && (() => {
                ...
                return (
                  <div style={{
                    padding: '8px 12px',
                    borderRadius: '6px',
                    background: overCount > 0 ? 'rgba(239, 68, 68, 0.08)' : 'var(--bg-secondary)',
                    border: overCount > 0 ? '1px solid rgba(239, 68, 68, 0.2)' : '1px solid var(--border-color)',
                    fontSize: '11px',
                    color: 'var(--text-secondary)',
                    lineHeight: 1.6,
                  }}>
                    ...
                    {overCount > 0 && (
                      <div style={{ color: '#ef4444', marginTop: '2px' }}>
                        {overCount} {overCount === 1 ? 'entity exceeds' : 'entities exceed'} the current output token limit — results may be truncated.
                      </div>
                    )}
                  </div>
                );
            ...
            <>
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 500 }}>
                    Entity {Math.min(progress.processedEntities + 1, progress.totalEntities)} / {progress.totalEntities}
                  </span>
              ...
              {progress.currentEntityName && !isTerminal && (
                <div style={{
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    {isClear ? 'Clearing annotations...' : isReview ? 'Generating annotations...' : 'Generating copy edit...'}
                  </div>
                </div>
              )}
              ...
              {progress.status === 'complete' && (
                <div style={{
                  ...
                  {progress.failedEntities.length > 0 && (
                    <span style={{ color: '#ef4444' }}> {progress.failedEntities.length} failed.</span>
                  )}
                </div>
              )}
              ...
              {progress.status === 'cancelled' && (
                <div style={{
                  ...
                  Cancelled after processing {progress.processedEntities} of {progress.totalEntities} entities.
                </div>
              )}
                  ...
                  <div style={{
                    ...
                    {progress.failedEntities.map((f, i) => (
                      <div
                        ...
                        style={{
                          padding: '6px 12px',
                          borderBottom: i < progress.failedEntities.length - 1 ? '1px solid var(--border-color)' : 'none',
                          fontSize: '11px',
                        }}
                      >
