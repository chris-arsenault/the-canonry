...
const ERA_NARRATIVE_TONES = [
  ...
  { value: 'cantankerous', label: 'Cantankerous', symbol: '♯', description: 'Irritable energy, argues with the dead' },
  { value: 'bemused', label: 'Bemused', symbol: '⁂', description: 'Puzzled and delighted by absurdity' },
  { value: 'defiant', label: 'Defiant', symbol: '▲', description: 'Proud of what was attempted' },
  { value: 'sardonic', label: 'Sardonic', symbol: '◆', description: 'Sharp irony, names the pattern' },
  ...
  { value: 'enthusiastic', label: 'Enthusiastic', symbol: '⚡', description: 'Thrilled by scale and ambition' },
];
...
export default function BulkEraNarrativeModal({
  ...
  useEffect(() => {
    if (isOpen && progress.status === 'idle' && chronicleItems?.length > 0 && wizardEras?.length > 0) {
      prepareBulk(chronicleItems, wizardEras, eraTemporalInfo, projectId, simulationRunId, 'witty', narrativeWeightMap);
    }
  }, [isOpen]);
  ...
  const globalPercent = progress.totalEras > 0
    ? Math.round((progress.processedEras / progress.totalEras) * 100)
    : 0;
                ...
                <button
                  onClick={() => useFloatingPillStore.getState().minimize({
                    ...
                    statusText: progress.status === 'running'
                      ? `${progress.processedEras}/${progress.totalEras}`
                      : progress.status,
                    statusColor: progress.status === 'running' ? '#f59e0b'
                      : progress.status === 'complete' ? '#10b981' : '#ef4444',
                    tabId: 'chronicle',
                  })}
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          padding: '6px 12px',
                          borderBottom: i < progress.eras.length - 1 ? '1px solid var(--border-color)' : 'none',
                          fontSize: '12px',
                        }}
                      >
                              ...
                              return (
                                <button
                                  ...
                                  style={{
                                    ...
                                    padding: 0,
                                    borderRadius: '3px',
                                    border: selected ? '1px solid #8b7355' : '1px solid transparent',
                                    background: selected ? 'rgba(139, 115, 85, 0.15)' : 'transparent',
              ...
              <div style={{ marginBottom: '16px' }}>
                <div style={{
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 500 }}>
                    Era {Math.min(progress.processedEras + 1, progress.totalEras)} / {progress.totalEras}
                  </span>
                ...
                return (
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {currentToneMeta && (
                        <span style={{ color: '#8b7355', fontSize: '13px' }} title={currentToneMeta.label}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {stepOrder.map((step, idx) => {
                        ...
                        const iconColor = isDone ? '#10b981' : isActive ? '#f59e0b' : 'var(--text-muted)';

                        // Word count bar caps for visual scaling
                        const barMax = step === 'generate' ? 4000 : step === 'threads' ? 2000 : 3000;
                        ...
                        return (
                          <div key={step}>
                            <div style={{
                              ...
                              <span style={{ color: iconColor, width: '12px', textAlign: 'center', fontWeight: 600, fontSize: '10px' }}>
                                {icon}
                              </span>
                              <span style={{
                                width: '52px',
                                  ...
                                  <span style={{ color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                                    {thinkingWords > 0 && (
                                      <span title="Thinking words received">
                                        <span style={{ color: 'var(--text-muted)', opacity: 0.6 }}>T</span>{' '}
                              ...
                              {isDone && (
                                <span style={{ flex: 1, fontSize: '10px', color: 'var(--text-muted)' }}>
                                  done
                                </span>
                              )}
                            </div>
              ...
              {(progress.totalCost > 0 || progress.totalWords > 0) && (
                <div style={{
                  ...
                  {progress.totalCost > 0 && (
                    <span>Cost: ${progress.totalCost.toFixed(4)}</span>
                  )}
                </div>
              )}
            </>
