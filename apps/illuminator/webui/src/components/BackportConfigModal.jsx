...
export default function BackportConfigModal({
  ...
  const renderEntityRow = (e) => {
    ...
    return (
      ...
      >
        {isLocked ? (
          <span style={{ width: '13px', textAlign: 'center', fontSize: '11px', color: status === 'backported' ? '#10b981' : 'var(--text-muted)' }}>
            {status === 'backported' ? '\u2713' : '\u2014'}
          </span>
        ) : (
          <input
        ...
        <span style={{ flex: 1, cursor: isLocked ? 'default' : 'pointer' }} onClick={() => !isLocked && toggleEntity(e.id)}>
          {e.name}
        ...
        {!status && (
          <button
            onClick={(ev) => { ev.stopPropagation(); handleMarkNotNeeded(e.id); }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
            <p style={{ margin: 0, fontSize: '11px', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
              {chronicleTitle}
            </p>
            <span style={{
              fontSize: '11px',
              fontWeight: 500,
              color: doneCount === entities.length ? '#10b981' : doneCount > 0 ? '#f59e0b' : 'var(--text-muted)',
              whiteSpace: 'nowrap',
              marginLeft: '8px',
            }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{
              ...
              <span style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>
                Entities ({selectedCount} selected{pendingEntities.length < entities.length ? `, ${doneCount} done` : ''})
              </span>
            ...
            <div style={{
              ...
              {tertiaryEntities.length > 0 && (castEntities.length > 0 || lensEntities.length > 0) && (
                <div style={{
                  borderTop: '1px solid var(--border-color)',
                  marginTop: '4px',
                  paddingTop: '4px',
