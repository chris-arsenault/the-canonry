...
function EntitySearchPanel({
  ...
  return <div className="qcm-search-panel">
      ...
      autoFocus className="qcm-search-input" />
      ...
      <div className="qcm-search-results">
        {query.trim().length < 2 && <div className="qcm-search-hint">Type at least 2 characters to search</div>}
        {query.trim().length >= 2 && results.length === 0 && <div className="qcm-search-hint">No matching entities found</div>}
        {query.trim().length >= 2 && results.length > 0 && results.slice(0, 10).map(({
        entity,
        matches
              ...
              <div className="qcm-search-result-header">
                <span className="qcm-search-result-name">
                  {matches.some(m => m.field === "name") ? <HighlightMatch text={entity.name} query={query} matchIndex={matches.find(m => m.field === "name").matchIndex} /> : entity.name}
                </span>
              ...
              {matches.filter(m => m.field !== "name").slice(0, 3).map((m, j) => <div key={j} className="qcm-search-match-row">
                    <span className="qcm-search-match-field">{m.field}</span>
                    <span className="qcm-search-match-value">
                      <HighlightMatch text={m.value} query={query} truncate={m.field === "summary" || m.field === "description" ? 120 : 0} matchIndex={m.matchIndex} />
                    </span>
                  </div>)}
            </div>)}
        {results.length > 10 && <div className="qcm-search-more">+{results.length - 10} more results</div>}
      </div>
    </div>;
}
...
function SuspectCard({
  ...
  return <div className="qcm-suspect-card"
  style={{
    "--qcm-confidence-color": CONFIDENCE_COLORS[suspect.confidence] || "#6b7280"
  }}>
      ...
      <div className="qcm-suspect-actions">
        {entities && <button onClick={() => setSearchOpen(!searchOpen)} className="illuminator-button illuminator-button-secondary qcm-suspect-action-btn" title="Search entity database for this reference">
            {searchOpen ? "Close Search" : "Search"}
          </button>}
        {onCreateEntity && <button onClick={() => onCreateEntity(suspect.phrase)} className="illuminator-button illuminator-button-secondary qcm-suspect-action-btn" title="Create a new entity from this reference">
            + Create
          </button>}
      </div>

      {/* Inline search panel */}
      {searchOpen && entities && <EntitySearchPanel entities={entities} initialQuery={suspect.phrase} onClose={() => setSearchOpen(false)} />}
    </div>;
}
