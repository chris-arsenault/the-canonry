  ...
  return (
    ...
    >
      ...
      <div style={{ maxHeight: '200px', overflow: 'auto' }}>
        {query.trim().length < 2 ? (
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', padding: '8px 4px', textAlign: 'center' }}>
          ...
          results.slice(0, 10).map(({ entity, matches }) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <span style={{ fontWeight: 600, fontSize: '12px' }}>
                  {matches.some(m => m.field === 'name')
                    ? <HighlightMatch text={entity.name} query={query} matchIndex={matches.find(m => m.field === 'name').matchIndex} />
                    : entity.name
                  }
                </span>
              ...
              {matches.filter(m => m.field !== 'name').slice(0, 3).map((m, j) => (
                <div key={j} style={{ marginTop: '2px', display: 'flex', gap: '4px', alignItems: 'baseline' }}>
                  <span style={{ fontSize: '9px', textTransform: 'uppercase', opacity: 0.7, flexShrink: 0 }}>
                    {m.field}
                  </span>
        ...
        {results.length > 10 && (
          <div style={{ fontSize: '10px', color: 'var(--text-muted)', padding: '4px', textAlign: 'center' }}>
