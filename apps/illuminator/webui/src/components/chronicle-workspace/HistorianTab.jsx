...
function BackportLoreButton({
  ...
  const {
    ...
    total
  } = useMemo(() => computeBackportProgress(item), [item.entityBackportStatus, item.roleAssignments, item.lens, item.tertiaryCast]);
  ...
  return <div className="htab-backport-row">
      <button onClick={onBackportLore} disabled={isGenerating} className="htab-backport-btn" title={tooltip}>
        {label}
      </button>
      {done > 0 && <span className={`htab-backport-status ${allDone ? "htab-backport-status-complete" : "htab-backport-status-partial"}`} title={`${done}/${total} entities backported`}>
          &#x21C4; {done}/{total}
        </span>}
    </div>;
}
