...
const normalizeFact = (fact, index) => {
  ...
  let text;
  if (typeof fact.text === "string") text = fact.text;else if (fact.text != null) text = String(fact.text);else text = "";
  return {
    id,
    text,
    type,
...
function WordCountHistogram({
  ...
  return <div className="cvp-histogram">
      ...
      <div className="cvp-histogram-bars">
        ...
        return <div key={bucket.label} className="cvp-histogram-bucket">
              {total > 0 && <span className="cvp-histogram-count">{total}</span>}
              <div title={`${bucket.label}: ${bucket.story} story, ${bucket.document} document`} className={`cvp-histogram-bar ${total === 0 ? "cvp-histogram-bar-empty" : "cvp-histogram-bar-filled"}`} style={total > 0 ? {
            "--cvp-bar-height": `${height}px`,
            "--cvp-story-pct": `${storyPct}%`
          } : undefined} />
      ...
      {expanded && <>
          ...
          <div className="illuminator-coverage-table">
            <table>
              ...
              return <tr key={chronicle.chronicleId} className={rowClass}>
                      ...
                      <td className="illuminator-coverage-toggle-col">
                        <input type="checkbox" checked={row.isIncluded} onChange={() => toggleChronicle(chronicle.chronicleId)} aria-label={`Toggle counting for ${chronicle.title || chronicle.chronicleId}`} />
                      </td>
                      {facts.map(fact => <td key={`${chronicle.chronicleId}-${fact.id}`} className={fact.disabled ? "cvp-cell-disabled" : undefined}>
                          {row.facetIds.has(fact.id) ? <span className="illuminator-coverage-hit" /> : <span className="illuminator-coverage-miss" />}
                        </td>)}
          ...
          {coverageStats && <div className="cvp-analysis">
              ...
              <div className="cvp-analysis-summary">
                {[{
            key: "integral",
            cls: "cvp-color-integral"
          }, {
            key: "prevalent",
            cls: "cvp-color-prevalent"
          }, {
            key: "mentioned",
            cls: "cvp-color-mentioned"
          }, {
            key: "missing",
            cls: "cvp-color-muted"
          }].map(({
            ...
            return <span key={key} className={cls}>
              ...
              let strengthClass;
              if (strength >= 60) strengthClass = "cvp-strength-high";else if (strength >= 30) strengthClass = "cvp-strength-mid";else strengthClass = "cvp-strength-low";
              return <tr key={fact.id}>
                        ...
                        <td className={`cvp-analysis-td-strength ${strengthClass}`}>
                          {strength}%
                        </td>
                      </tr>;
            })}
