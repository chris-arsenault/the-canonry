...
function SpecialToggle({
  ...
  return <div className="isd-special-toggle-row">
      {[{
      id: RANDOM_ID,
      label: "\u2684 Random"
    }, {
      id: NONE_ID,
      label: "\u2014 None"
    }].map(opt => <button key={opt.id} onClick={() => onChange(opt.id)} className="isd-special-toggle-btn" data-selected={value === opt.id}>
          {opt.label}
        </button>)}
...
function CollapsibleSection({
  ...
  return <div className="isd-section">
...
function SwatchStrip({
  ...
  return <div className="isd-swatch-strip">
      {colors.map((color, i) => <div key={i} className="isd-swatch" style={{
      "--swatch-bg": color
          ...
          {!styleLibrary ? <div className="isd-loading">Loading styles...</div> : <>
              ...
              <CollapsibleSection title="Output" sectionKey="output" collapsed={collapsedSet.has("output")} onToggle={toggleSection}>
                ...
                {cultures && cultures.length > 0 && <div>
                    ...
                    <select value={settings.selectedCultureId} onChange={e => onSettingsChange({
                selectedCultureId: e.target.value
              })} className="illuminator-select isd-culture-select">
                      <option value="">Auto-detect</option>
                      {cultures.map(culture => <option key={culture.id} value={culture.id}>
                          {culture.name}
                        </option>)}
                    </select>
                  </div>}
