...
export default function EraNarrativeModal({
  ...
  return <div className="era-narr-overlay" onClick={e => {
      ...
      <div className="era-narr-modal">
        ...
        <div className="era-narr-header">
          ...
          <div className="era-narr-header-actions">
            {isActive && !isComplete && <button onClick={() => useFloatingPillStore.getState().minimize({
            id: PILL_ID,
            label: `Era: ${narrative?.eraName || "Narrative"}`,
            statusText: isGenerating ? narrative?.currentStep || "Working" : "Review",
            statusColor: isGenerating ? "#f59e0b" : "#3b82f6",
            tabId: "chronicle"
          })} className="illuminator-button era-narr-header-minimize-btn" title="Minimize to pill">
                ―
              </button>}
        ...
        <div className="era-narr-body">
          ...
          {showSetup && <>
              <div className="era-narr-field">
                ...
                <select id="era" className="illuminator-select era-narr-select-full" value={selectedEraId} onChange={e => setSelectedEraId(e.target.value)}>
                  <option value="">Select an era...</option>
                  {eraOptions.map(era => <option key={era.id} value={era.id}>
                      {era.name} ({era.count} chronicles, {era.preppedCount}/{era.count} prepped)
                    </option>)}
                </select>
              </div>

              {selectedEra && selectedEra.preppedCount === 0 && <div className="era-narr-warning">
                  No chronicles in this era have historian prep briefs. Run Historian Prep first.
                </div>}

              {selectedEra && selectedEra.preppedCount > 0 && selectedEra.preppedCount < selectedEra.count && <div className="era-narr-warning">
              ...
              {previousEraThesis && <div className="era-narr-thesis-link">
              ...
              {eraChronicles.length > 0 && <div className="era-narr-field">
                ...
                let weightSymbol;
                if (weight === "structural") weightSymbol = "\u25A0";else if (weight === "contextual") weightSymbol = "\u25A1";else if (weight === "flavor") weightSymbol = "\u25CB";else weightSymbol = "\u2013";
                let weightColor;
                if (weight === "structural") weightColor = "#3b82f6";else if (weight === "contextual") weightColor = "#f59e0b";else if (weight === "flavor") weightColor = "#a855f7";else weightColor = "var(--text-muted)";
                return <div key={c.chronicleId} className={`era-narr-chronicle-item${c.hasHistorianPrep ? "" : " era-narr-chronicle-item-unprepped"}`}>
                          <span className="era-narr-chronicle-item-weight" title={weight ? `Weight: ${weight}` : "No weight assigned"} style={{
                    "--era-narr-weight-color": weightColor
                  }}>
                            {weightSymbol}
                          </span>
                          ...
                          {c.eraYear != null && <span className="era-narr-chronicle-item-year" title="Era year">
                              Y{c.eraYear}
                            </span>}
                          {c.narrativeStyleName && <span className="era-narr-chronicle-item-style" title={`Style: ${c.narrativeStyleName}`}>
                              {c.narrativeStyleName}
                            </span>}
                          <span className={`era-narr-chronicle-item-prep${c.hasHistorianPrep ? " era-narr-chronicle-item-prep-yes" : ""}`} title={c.hasHistorianPrep ? "Has historian prep brief" : "No prep brief"}>
                            {c.hasHistorianPrep ? "\u270E" : "\u2013"}
                          </span>
                        </div>;
              })}
              ...
              {existingNarratives.length > 0 && <div className="era-narr-field">
                ...
                let statusIcon;
                if (rec.status === "complete") statusIcon = "\u2713";else if (rec.status === "failed") statusIcon = "\u2717";else if (rec.status === "step_complete") statusIcon = "\u25CB";else statusIcon = "\u2026";
                let statusColor;
                if (rec.status === "complete") statusColor = "#10b981";else if (rec.status === "failed") statusColor = "#ef4444";else statusColor = "#f59e0b";
                ...
                return <div key={rec.narrativeId} className="era-narr-existing-item">
                          {}
                          <span className="era-narr-existing-item-status" title={rec.status} style={{
                    "--era-narr-status-color": statusColor
                  }}>
                            {statusIcon}
                          </span>
                          ...
                          {rec.totalActualCost > 0 && <span className="era-narr-existing-item-cost">
                              ${rec.totalActualCost.toFixed(3)}
                            </span>}
                          {canResume && <button onClick={() => handleResume(rec.narrativeId)} className="illuminator-button era-narr-existing-item-resume-btn">
                              Resume
                            </button>}
                          {!canResume && <button onClick={() => handleResume(rec.narrativeId)} className="illuminator-button era-narr-existing-item-view-btn">
                              View
                            </button>}
                          <button onClick={() => void handleDeleteExisting(rec.narrativeId)} className="illuminator-button era-narr-existing-item-delete-btn" title="Delete narrative">
                            {"\u2715"}
                          </button>
                        </div>;
              })}
              ...
              <div className="era-narr-field-lg">
                ...
                <div className="era-narr-tone-row">
                  {TONE_OPTIONS.map(t => <button key={t.value} onClick={() => setTone(t.value)} title={t.description} className={`illuminator-button era-narr-tone-btn ${tone === t.value ? "era-narr-tone-btn-active" : ""}`}>
                      {t.label}
                    </button>)}
                </div>
              </div>
              ...
              <div className="era-narr-field-lg">
                ...
                <textarea id="arc-direction" value={arcDirection} onChange={e => setArcDirection(e.target.value)} placeholder="Override the era's narrative arc. When set, the historian's thesis, thread arcs, and register choices must honor this direction." className="era-narr-textarea" />
              </div>

              <div className="era-narr-start-row">
                <button onClick={() => void handleStart()} disabled={!selectedEraId || !selectedEra || selectedEra.preppedCount === 0} className={`illuminator-button era-narr-start-btn ${selectedEraId && selectedEra?.preppedCount > 0 ? "era-narr-start-btn-primary" : "era-narr-start-btn-dimmed"}`}>
                  Start Narrative
                </button>
                <button onClick={() => void handleStartHeadless()} disabled={!selectedEraId || !selectedEra || selectedEra.preppedCount === 0} className={`illuminator-button era-narr-headless-btn${selectedEraId && selectedEra?.preppedCount > 0 ? "" : " era-narr-start-btn-dimmed"}`} title="Run all steps without pausing for review">
                  Run Headless
                </button>
              </div>
            </>}
          ...
          {/* Generating */}
          {isGenerating && <div className="era-narr-generating">
          ...
          {showThreadReview && synthesis && <>
              ...
              <div className="era-narr-review-section">
                ...
                <div className="era-narr-thread-list">
                  {synthesis.threads.map(t => <div key={t.threadId} className="era-narr-thread-card">
                      <div className="era-narr-thread-card-header">
                        <span className="era-narr-thread-card-name">{t.name}</span>
                        {t.culturalActors?.length > 0 && <span className="era-narr-thread-card-actors">
                      ...
                      {t.material && <details className="era-narr-thread-card-material">
              ...
              {synthesis.movements?.length > 0 && <div className="era-narr-review-section">
                  ...
                  <div className="era-narr-movement-list">
                    {synthesis.movements.map(m => <div key={m.movementIndex} className="era-narr-movement-card">
              ...
              {synthesis.counterweight && <div className="era-narr-review-section">
                  <div className="era-narr-review-heading">Counterweight</div>
                  <div className="era-narr-counterweight">{synthesis.counterweight}</div>
                </div>}

              {synthesis.strategicDynamics?.length > 0 && <div className="era-narr-review-section">
                  <div className="era-narr-review-heading">
                    Strategic Dynamics ({synthesis.strategicDynamics.length})
                  </div>
                  <div className="era-narr-dynamics-list">
                    {synthesis.strategicDynamics.map((sd, i) => <div key={i} className="era-narr-dynamic-card">
              ...
              {synthesis.quotes?.length > 0 && <div className="era-narr-review-section">
                  <div className="era-narr-review-heading">Quotes ({synthesis.quotes.length})</div>
                  <div className="era-narr-quotes-list">
                    {synthesis.quotes.map((q, i) => <div key={i} className="era-narr-quote-card">
              ...
              {synthesis.motifs?.length > 0 && <div className="era-narr-review-section">
          ...
          {isComplete && <>
              {/* Version Selector */}
              {resolved.versions.length > 0 && <div className="era-narr-version-row">
                  <select value={selectedVersionId || resolved.activeVersionId || ""} onChange={e => {
              ...
              setConfirmingDeleteId(null);
                ...
                return <option key={v.versionId} value={v.versionId}>
                  ...
                  {viewedVersion && viewedVersion.versionId === resolved.activeVersionId && <span className="era-narr-version-active-badge">Active</span>}
                  {viewedVersion && viewedVersion.versionId !== resolved.activeVersionId && <button onClick={() => {
              ...
              }} onBlur={() => setConfirmingDeleteId(null)} className={`illuminator-button era-narr-version-delete-btn ${isConfirming ? "era-narr-version-delete-btn-confirming" : "era-narr-version-delete-btn-normal"}`} title={isConfirming ? "Click again to confirm" : "Delete this version"}>
              ...
              {/* Content viewer */}
              {viewedContent ? <div className="era-narr-content-viewer">{viewedContent}</div> : <div className="era-narr-no-content">No content available</div>}
            </>}
        </div>

        {/* Footer */}
        {(showThreadReview || showNarrativeReview || showEditReview || isComplete) && <div className="era-narr-footer">
            ...
            <div className="era-narr-footer-actions">
              {!isComplete && <button onClick={cancel} className="illuminator-button">
                  Cancel
                </button>}

              {showThreadReview && <button onClick={advanceStep} className="illuminator-button era-narr-footer-primary-btn">
                  Generate Narrative
                </button>}
              ...
              {showEditReview && <button onClick={advanceStep} className="illuminator-button era-narr-footer-primary-btn">
                  Finish
                </button>}
              ...
              rerunCopyEdit();
            }} className="illuminator-button era-narr-footer-rerun-btn" title="Re-run the copy edit pass on the latest version">
                    Re-run Copy Edit
                  </button>
