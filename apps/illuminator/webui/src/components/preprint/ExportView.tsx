...
const FORMAT_CONTENTS: Record<ExportFormat, { label: string; description: string }[]> = {
  markdown: [
    {
      label: "manifest.json",
      description: "Full metadata: stats, tree structure, image inventory",
    },
    { label: "Folder hierarchy", description: "Matching your content tree structure" },
    {
      label: "Markdown files",
      description: "YAML frontmatter + formatted content + historian notes",
    },
  ],
  indesign: [
    {
      label: "One Story per entry",
      description:
        "Each entity, chronicle, era narrative gets its own Story \u2014 reorder or delete entries independently",
    },
    {
      label: "4 master spreads",
      description:
        "A-Story, B-Document, C-Narrative, D-Encyclopedia \u2014 override per content type",
    },
    {
      label: "Inline footnotes",
      description: "Minor historian notes as footnotes at anchor positions",
    },
    { label: "Callout boxes", description: "Major historian notes as separate text frames" },
    {
      label: "Linked images",
      description:
        "Image placeholders linked to images/ \u2014 use the Download Image Script button to pull from S3",
    },
  ],
};
          ...
          <div className="preprint-export-config">
            ...
            {idmlLayout.columnCount === 2 && (
              <div className="preprint-stats-row">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setIdmlLayout((prev) => ({ ...prev, columnGutter: Number(e.target.value) }))
                  }
                >
                  {[12, 18, 24].map((g) => (
            ...
            <div className="preprint-stats-row">
              ...
              <select
                ...
                onChange={(e) =>
                  setIdmlLayout((prev) => ({ ...prev, paragraphSpacing: Number(e.target.value) }))
                }
              >
