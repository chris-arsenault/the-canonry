...
function AnchorEditor({
  ...
  return <div className="hmn-anchor-editor">
      <div className="hmn-anchor-editor-row">
        <input type="text" value={value} onChange={e => setValue(e.target.value)} className="hmn-anchor-input"
      ...
      autoFocus onKeyDown={e => {
        ...
        <button onClick={() => resolved && onSave(resolved.phrase)} disabled={!resolved} className={`hmn-anchor-save-btn${resolved ? " hmn-anchor-save-btn-resolved" : ""}`}>
          Save
        </button>
      ...
      {/* Resolution status */}
      <div className={`hmn-anchor-status ${resolved ? "hmn-anchor-status-resolved" : "hmn-anchor-status-unresolved"}`}>
        {(() => {
...
function NoteItem({
  ...
  const handleSaveAnchor = useCallback(newPhrase => {
    if (!onUpdateNote) return;
    onUpdateNote(note.noteId, {
      anchorPhrase: newPhrase
    });
    setEditingAnchor(false);
  }, [onUpdateNote, note.noteId]);
  const isDisabled = display === "disabled";
  return <div className={`hmn-note-item${isDisabled ? " hmn-note-item-disabled" : ""}`} style={{
    "--hmn-note-color": meta.color
  }}>
      {/* Display mode toggle */}
      {onUpdateNote && <button onClick={cycleDisplay} title={`${DISPLAY_LABELS[display]} \u2014 click to cycle (full \u2192 popout \u2192 disabled)`} className={`hmn-display-toggle${isDisabled ? " hmn-display-toggle-disabled" : ""}`}>
      ...
      <div className="hmn-note-content">
        ...
        <div className="hmn-note-header">
          <span className={`hmn-type-label${isDisabled ? " hmn-type-label-disabled" : ""}`}>
            {meta.icon} {meta.label}
          </span>
          {display !== "full" && <span className="hmn-display-label">{DISPLAY_LABELS[display]}</span>}
          <span onClick={() => !editingAnchor && setEditingAnchor(true)} className={`hmn-anchor-text ${anchorMissing ? "hmn-anchor-text-missing" : "hmn-anchor-text-normal"} ${onUpdateNote ? "hmn-anchor-text-editable" : "hmn-anchor-text-readonly"}`} title={`Anchor: "${note.anchorPhrase}"${anchorMissing ? " (not found in source text)" : ""}${onUpdateNote ? " — click to edit" : ""}`} role="button" tabIndex={0} onKeyDown={e => {
        ...
        {anchorMissing && !editingAnchor && <div className="hmn-anchor-warning">Anchor text not found in source</div>}

        {/* Anchor editor */}
        {editingAnchor && onUpdateNote && <AnchorEditor anchorPhrase={note.anchorPhrase} sourceText={sourceText} onSave={handleSaveAnchor} onCancel={() => setEditingAnchor(false)} />}

        {/* Note text */}
        <div className={`hmn-note-text ${isDisabled ? "hmn-note-text-disabled" : "hmn-note-text-active"}`} onClick={() => setExpanded(!expanded)} role="button" tabIndex={0} onKeyDown={e => {
        if (e.key === "Enter" || e.key === " ") e.currentTarget.click();
      }}>
