...
function CultureIdentityEditor({
  ...
  return <div className="illuminator-visual-identity-editor">
      ...
      <div className="illuminator-kind-selector vip-kind-selector-spaced">
        ...
        return <button key={culture.id} onClick={() => setSelectedCulture(culture.id)} className={`illuminator-kind-button ${selectedCulture === culture.id ? "active" : ""} ${warnings ? "has-warning" : ""}`}
        style={{
          "--vip-culture-color": culture.color || "var(--accent-color)"
        }}>
              {culture.name || culture.id}
              {warnings && <span className="illuminator-warning-badge" title={`${warnings} missing value${warnings > 1 ? "s" : ""}`}>
                  {warnings}
                </span>}
            </button>;
      })}
      ...
      {selectedCulture && <>
          {displayEntries.length > 0 && <div className="illuminator-visual-identity-entries">
              {displayEntries.map(({
          key,
          value,
          isEmpty
        }) => <div key={key} className={`illuminator-visual-identity-entry ${isEmpty ? "is-empty" : ""}`}>
                  <span className="illuminator-visual-identity-key">{key}</span>
                  <input type="text" className={`illuminator-input vip-input-flex ${isEmpty ? "warning" : ""}`} value={value} onChange={e => handleUpdateEntry(key, e.target.value)} placeholder={valuePlaceholder} />
                  {deleteConfirm === key ? <div className="illuminator-delete-confirm">
                      <button className="illuminator-button-small" onClick={() => handleDeleteSingle(key)} title="Delete from this culture only">
                        This
                      </button>
                      <button className="illuminator-button-small danger" onClick={() => handleDeleteAll(key)} title="Delete from all cultures">
                        All
                      </button>
                      <button className="illuminator-button-link vip-cancel-button" onClick={() => setDeleteConfirm(null)}>
                        x
                      </button>
                    </div> : <button className="illuminator-button-link vip-delete-button" onClick={() => handleRequestDelete(key)} title="Delete">
                      x
                    </button>}
                </div>)}
            </div>}
          ...
          <div className="illuminator-visual-identity-add">
            <input type="text" className="illuminator-input vip-key-input" placeholder={keyPlaceholder} value={newKey} onChange={e => setNewKey(e.target.value.toUpperCase())} />
            <input type="text" className="illuminator-input vip-input-flex" placeholder={valuePlaceholder} value={newValue} onChange={e => setNewValue(e.target.value)} />
            <button className="illuminator-button" onClick={handleAddEntry} disabled={!newKey.trim()}>
              Add
            </button>
          </div>

          {allKeys.length > 0 && <div className="illuminator-template-section-hint vip-hint-spaced">
...
function IdentityKeySelector({
  ...
  if (entityKinds.length === 0) {
    return <div className="illuminator-template-section-hint">No entity kinds defined in schema.</div>;
  }
  return <div>
      <div className="illuminator-kind-selector vip-kind-selector-spaced">
        {entityKinds.map(kind => {
        const count = keysByKind?.[kind.kind]?.length || 0;
        return <button key={kind.kind} onClick={() => setSelectedKind(kind.kind)} className={`illuminator-kind-button ${selectedKind === kind.kind ? "active" : ""}`}>
      ...
      {selectedKind && availableKeys.length > 0 && <div className="illuminator-kind-selector">
          {availableKeys.map(key => {
        const isSelected = selectedKeys.includes(key);
        return <button key={key} onClick={() => handleToggleKey(key)} className={`illuminator-kind-button vip-key-button-uppercase ${isSelected ? "active" : ""}`}>
                {isSelected ? "+ " : ""}
                {key}
              </button>;
      })}
      ...
      {selectedKind && availableKeys.length === 0 && <div className="illuminator-template-section-hint">{emptyMessage}</div>}
    </div>;
}
...
function IdentitySection({
  ...
  return <div className="illuminator-identity-section">
