...
export default function App() {
  ...
  const handleExportModalClick = useCallback(
    (e) => {
  ...
  useEffect(() => {
    ...
    Promise.all([loadWorldStore(currentProject.id), getSlots(currentProject.id)]).then(
      ([store, loadedSlots]) => {
        if (cancelled) return;
  ...
  const handleSearchRunScored = useCallback(
    (payload = {}) => {
      const {
        runScore,
        runScoreMax,
  ...
  const mergeFrameworkOverrides = (items, existingItems, frameworkKeys, keyField) => {
    ...
    const existingOverrides = (existingItems || []).filter((item) =>
      frameworkKeys.has(item?.[keyField])
    );
  ...
  const updateSeedEntities = useCallback((seedEntities) => save({ seedEntities }), [save]);
  ...
  const updateEras = useCallback((eras) => save({ eras }), [save]);

  const updatePressures = useCallback((pressures) => save({ pressures }), [save]);

  const updateGenerators = useCallback((generators) => save({ generators }), [save]);

  const updateSystems = useCallback((systems) => save({ systems }), [save]);

  const updateActions = useCallback((actions) => save({ actions }), [save]);
  ...
  const updateAxisDefinitions = useCallback((axisDefinitions) => save({ axisDefinitions }), [save]);
  ...
  // Update slot title
  const handleUpdateSlotTitle = useCallback(
    async (slotIndex, title) => {
      if (!currentProject?.id) return;
  ...
  const importSlotPayload = useCallback(
    async (slotIndex, payload, options = {}) => {
      if (!currentProject?.id) return;
      ...
      const simulationState = mergeDefined(
        parsed.simulationState,
        existingSlot.simulationState ?? null
      );
      ...
      if (
        simulationRunId &&
        Array.isArray(worldData?.hardState) &&
        worldData.hardState.length > 0
      ) {
      ...
      if (
        simulationRunId &&
        Array.isArray(worldData?.narrativeHistory) &&
        worldData.narrativeHistory.length > 0
      ) {
        const eventResult = await importNarrativeEvents(
          simulationRunId,
          worldData.narrativeHistory
        );
      ...
      if (
        parsed.staticPages?.length ||
        parsed.chronicles?.length ||
        parsed.imageData ||
        worldData?.hardState?.length
      ) {
        const [staticPages, chronicleCount, imageCount, entityCount, eventCount] =
          await Promise.all([
  ...
  const handleExportBundle = useCallback(
    async (slotIndex) => {
      ...
      try {
        ...
        const imageStorage = useS3Images
          ? buildImageStorageConfig(awsConfig, currentProject.id)
          : null;
  ...
  const awsReady = Boolean(
    awsConfig?.identityPoolId && awsConfig?.region && (!awsLoginConfigured || hasAwsToken)
  );
        ...
        >
          <div className="modal modal-simple">
            ...
            <div className="modal-actions">
              {isExportingBundle ? (
                <button className="btn-sm" onClick={handleCancelExportBundle}>
                  Cancel Export
                </button>
              ) : (
                <>
        ...
        >
          <div className="modal modal-simple" style={{ maxWidth: "900px" }}>
            ...
            <div className="modal-body">
              ...
              <div style={{ marginBottom: spacing.lg }}>
                <div className="modal-title" style={{ fontSize: typography.sizeMd }}>
                  Cognito Auth
                </div>
              ...
              <div style={{ marginBottom: spacing.lg }}>
                ...
                {awsBrowseState.loading && (
                  <div style={{ color: colors.textMuted, marginTop: spacing.sm }}>
                    Loading prefixes...
                  </div>
                )}
                {awsBrowseState.error && (
                  <div style={{ color: colors.danger, marginTop: spacing.sm }}>
                    {awsBrowseState.error}
                  </div>
                )}
