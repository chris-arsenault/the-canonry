...
const getHeatBucket = value => {
  if (value === 0 || value === undefined || value === null) return "none";
  ...
  return <div className="mat-layout weight-matrix-editor">
          ...
          <div className="orphan-warning-content">
            ...
            <div className="orphan-warning-text">
              ...
              <span className="orphan-warning-details">
                {orphanedReferences.generators.length > 0 && <span>
                    {orphanedReferences.generators.length} deleted generator
                    {orphanedReferences.generators.length !== 1 ? "s" : ""}
                  </span>}
                {orphanedReferences.generators.length > 0 && orphanedReferences.systems.length > 0 && ", "}
                {orphanedReferences.systems.length > 0 && <span>
                    {orphanedReferences.systems.length} deleted system
                    {orphanedReferences.systems.length !== 1 ? "s" : ""}
                  </span>}
      ...
      <div className="mat-toolbar matrix-toolbar">
        <div className="toolbar-left">
          ...
          <div className="view-toggle">
            <button className={`toggle-btn ${viewMode === "generators" ? "active" : ""}`} onClick={() => setViewMode("generators")}>
              Generators ({generators.length})
              {orphanedReferences.generators.length > 0 && viewMode !== "generators" && <span className="orphan-badge">{orphanedReferences.generators.length}</span>}
            </button>
            <button className={`toggle-btn ${viewMode === "systems" ? "active" : ""}`} onClick={() => setViewMode("systems")}>
              Systems ({systems.length})
              {orphanedReferences.systems.length > 0 && viewMode !== "systems" && <span className="orphan-badge">{orphanedReferences.systems.length}</span>}
            </button>
          </div>
        ...
        <div className="toolbar-right">
          {/* Bulk Actions */}
          {selectedRows.size > 0 && <div className="bulk-actions">
              <span className="selection-count">{selectedRows.size} selected</span>
              <NumberInput className="quick-set-input" value={quickSetValue} onChange={setQuickSetValue} min={0} max={10} step={0.1} />
              <button className="btn btn-secondary" onClick={handleQuickSetSelected}>
                Set All Eras
              </button>
              <button className="btn btn-ghost" onClick={() => setSelectedRows(new Set())}>
                Clear
              </button>
            </div>}

          {/* Paste indicator */}
          {copiedRow && <span className="copied-indicator">Copied: {copiedRow.id}</span>}
        </div>
      </div>
          ...
          <thead>
            <tr>
              <th className="checkbox-col">
                <input type="checkbox" checked={selectedRows.size === filteredItems.length && filteredItems.length > 0} onChange={selectAllRows} />
              </th>
              <th className="name-col">{viewMode === "generators" ? "Generator" : "System"}</th>
              <th className="status-col">Eras</th>
              {eras.map(era => <th key={era.id} className="era-col" title={era.summary || era.name}>
                  {era.name || era.id}
                </th>)}
          ...
          <tbody>
            {filteredItems.length === 0 ? <tr>
                <td colSpan={eras.length + 4} className="empty-row">
                  {searchQuery ? `No ${viewMode} match "${searchQuery}"` : `No ${viewMode} defined`}
                </td>
            ...
            return <tr key={item.id} className={`mat-row matrix-row ${!isUsed ? "unused" : ""} ${selectedRows.has(item.id) ? "selected" : ""}`}>
                    ...
                    <td className="name-col">
                      ...
                      {item.type && viewMode === "systems" && <span className="item-type">{item.type}</span>}
                    </td>
                ...
                return <td key={era.id} className={`weight-cell weight-cell-${getHeatBucket(weight)} ${weight === null ? "empty" : ""}`} onClick={() => handleCellClick(item.id, era.id)}>
                          {isEditing ? <input ref={inputRef} type="text" className="cell-input" value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={handleCellBlur} onKeyDown={handleKeyDown}
                  // eslint-disable-next-line jsx-a11y/no-autofocus
                  autoFocus /> : <span className={`cell-value cell-value-${weight === 0 || weight === undefined || weight === null ? "muted" : "active"}`}>
                              {weight !== null ? weight : "-"}
                            </span>}
                        </td>;
              })}
                    <td className="actions-col">
                      <div className="row-actions">
                        <button className="action-btn" onClick={() => applyToAllEras(item.id, 1)} title="Add to all eras (weight 1)">
                          +All
                        </button>
                        <button className="action-btn" onClick={() => copyRow(item.id)} title="Copy weights">
                          Copy
                        </button>
                        {copiedRow && copiedRow.id !== item.id && <button className="action-btn" onClick={() => pasteRow(item.id)} title="Paste weights">
                            Paste
                          </button>}
                        <button className="action-btn danger" onClick={() => removeFromAllEras(item.id)} title="Remove from all eras">
                          Clear
                        </button>
                      </div>
                    </td>
      ...
      <div className="mat-legend matrix-legend">
        ...
        <div className="legend-scale">
          ...
          <span className="legend-item weight-cell-low">
            2.5
          </span>
          <span className="legend-item weight-cell-mid">
            5
          </span>
          <span className="legend-item weight-cell-high">
            7.5
          </span>
          <span className="legend-item weight-cell-max">
            10
          </span>
        </div>
