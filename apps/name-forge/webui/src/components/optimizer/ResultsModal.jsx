...
function ResultRow({
  ...
  return <React.Fragment key={result.domainId}>
      <tr onClick={result.success ? () => onToggle(result.domainId) : undefined} className={`results-row ${isExpanded ? "expanded" : ""} ${result.success ? "clickable" : ""}`}>
        <td className="col-expand">
          {result.success && <span className="text-small">{isExpanded ? "▼" : "▶"}</span>}
        </td>
        <td>
          {result.success ? <span className="text-success mr-sm">✓</span> : <span className="text-danger mr-sm">✗</span>}
          {result.domainId}
        </td>
      ...
      {isExpanded && result.success && <tr key={`${index}-diff`}>
